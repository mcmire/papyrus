...
Expectations do
  ...
  for method in %w(clear clear_cache)
    expect Hash.new do
      context_item = PageTemplate::ContextItemWrapper.new
  ...
  expect PageTemplate::ContextItemWrapper.new.to.receive(:get_primary_part).with('foo', 'foo') do |context_item|
...
=end
  ...
  expect Mocha::Mock do
    context_item = PageTemplate::ContextItemWrapper.new
    PageTemplate::Parser.stubs(:recent_parser).returns stub('parser')
