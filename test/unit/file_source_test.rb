...
Expectations do
  ...
  expect(:include_paths => nil) do
    source = Papyrus::FileSource.new('include_paths' => nil)
  ...
  expect nil do
    source = Papyrus::FileSource.new(:include_paths => %w(/some))
  ...
  expect '/some/file' do
    source = Papyrus::FileSource.new(:include_paths => %w(/different))
    File.stubs(:exists?).returns(true)
    File.stubs(:expand_path).returns('/some/file')
    source.get_filename('/some/file')
  end
