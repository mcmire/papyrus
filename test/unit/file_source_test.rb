...
Expectations do
  ...
  expect(:include_paths => nil) do
    source = PageTemplate::FileSource.new('include_paths' => nil)
  ...
  expect([ 'foo', 'bar', 'baz' ]) do
    source = PageTemplate::FileSource.new(:include_paths => ['foo', 'bar', 'baz'])
    source.send :instance_variable_get, "@paths"
  end
  # #initialize should set @paths to [ :include_path ] if that option is passed
  expect([ 'foo' ]) do
    source = PageTemplate::FileSource.new(:include_path => 'foo')
    source.send :instance_variable_get, "@paths"
  end
  # #initialize should set @paths to [ Dir.getwd, '/' ] by default
  expect([ Dir.getwd, '/' ]) do
    source = PageTemplate::FileSource.new
    source.send :instance_variable_get, "@paths"
  end
  # #initialize should compact @paths
  expect([ 'foo', 'bar' ]) do
    source = PageTemplate::FileSource.new(:include_paths => ['foo', nil, 'bar' ])
    source.send :instance_variable_get, "@paths"
  end
  ...
  expect nil do
    source = PageTemplate::FileSource.new(:include_paths => %w(/some))
  ...
  expect '/some/file' do
    source = PageTemplate::FileSource.new(:include_paths => %w(/different))
    File.stubs(:exists?).returns(true)
    File.stubs(:expand_path).returns('/some/file')
    source.get_filename('/some/file')
  end
