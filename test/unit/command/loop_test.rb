...
Expectations do
  ...
  begin
    expect 42 do
      loop_cmd = PageTemplate::Command::Loop.new(nil, "", 42, "")
      loop_cmd.send(:instance_variable_get, "@value")
    end
    expect ['foo', 'bar', 'baz'] do
      loop_cmd = PageTemplate::Command::Loop.new(nil, "", "", "foo bar baz")
      loop_cmd.send(:instance_variable_get, "@block_params")
    end
    expect false do
      loop_cmd = PageTemplate::Command::Loop.new(nil, "", "", "")
  ...
  begin
    expect PageTemplate::Command::Block do
      loop_cmd = PageTemplate::Command::Loop.new(nil, "", "", "")
      loop_cmd.send(:instance_variable_set, "@in_else", true)
      loop_cmd << PageTemplate::Command::Block.new
  ...
  begin
    expect PageTemplate::Command::Loop.new(nil, "", "", "").to.receive(:set_block_params) do |loop_cmd|
      loop_cmd.send(:create_subcontext, nil, nil, nil, nil)
    end
    expect PageTemplate::Command::Loop.new(nil, "", "", "").to.receive(:set_metavariables) do |loop_cmd|
      loop_cmd.send(:create_subcontext, nil, nil, nil, nil)
    end
  end
  ...
  expect "[ Loop: foobarbazquux [ Blocks: [[ filter ]] [blah blah] ] Else: [ Blocks: [[ filter ]] [blah blah] ] ]" do
    ...
    main_block << PageTemplate::Command::Filter.new(nil, "filter", :unescaped)
    ...
    else_block << PageTemplate::Command::Filter.new(nil, "filter", :unescaped)
