...
Expectations do
  ...
  begin
    ...
    expect ['foo', 'bar', 'baz'] do
      loop_cmd = PageTemplate::Command::Loop.new("", "", "foo bar baz")
  ...
  begin
    expect PageTemplate::Command::Loop.new("", "", "").to.receive(:set_block_params) do |loop_cmd|
      loop_cmd.send(:create_subcontext, nil, nil, nil, nil)
    end
    expect PageTemplate::Command::Loop.new("", "", "").to.receive(:set_metavariables) do |loop_cmd|
      loop_cmd.send(:create_subcontext, nil, nil, nil, nil)
    end
  end
