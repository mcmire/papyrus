...
Expectations do
  ...
  # Include#initialize
  expect "foo" do
    include_cmd = PageTemplate::Command::Include.new("", "foo")
  ...
  begin
    expect "Output of template" do
      include_cmd = PageTemplate::Command::Include.new("", "")
  ...
  begin
    ...
    expect stub('source').to.receive(:get).with("some_file.txt") do |source|
      include_cmd = PageTemplate::Command::Include.new("", "some_file.txt")
