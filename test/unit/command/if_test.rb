...
Expectations do
  ...
  begin
    expect 42 do
      if_cmd = PageTemplate::Command::If.new(nil, "", 42)
      if_cmd.send(:instance_variable_get, "@value")
    end
    expect [ 42, PageTemplate::Command::Block ] do
      if_cmd = PageTemplate::Command::If.new(nil, "", 42)
    ...
    expect PageTemplate::Command::Block do
      if_cmd = PageTemplate::Command::If.new(nil, "", "")
      if_cmd.send(:instance_variable_get, "@false_commands")
    end
    # @in_else
    begin
      expect false do
        if_cmd = PageTemplate::Command::If.new(nil, "if", "")
        if_cmd.send(:instance_variable_get, "@in_else")
      end
      expect true do
        if_cmd = PageTemplate::Command::If.new(nil, "unless", "")
        if_cmd.send(:instance_variable_get, "@in_else")
      end
    end
    expect false do
  ...
  begin
    ...
    expect PageTemplate::Command::Base do
      ...
      if_cmd << PageTemplate::Command::Base.new
  ...
  begin
    ...
    expect "[ If (42) [ Blocks: [foo bar] ] Elsif (foo) [ filter ] Else: [ Blocks: [bar baz] ] ]" do
      ...
      if_cmd.send(:instance_variable_set, "@true_commands", [
        ...
        [ 'foo', PageTemplate::Command::Filter.new(nil, "filter", :unescaped) ]
      ])
    ...
    expect "[ Unless (42): [ Blocks: [bar baz] ] Else: [ Blocks: [foo bar] ] ]" do
      if_cmd = PageTemplate::Command::If.new(nil, "unless", 42)
      if_cmd.send(:instance_variable_set, "@false_commands",
        (PageTemplate::Command::Block.new << PageTemplate::Command::Text.new("bar baz")) )
      if_cmd.send(:instance_variable_set, "@true_commands", [
        [ nil, (PageTemplate::Command::Block.new << PageTemplate::Command::Text.new("foo bar")) ]
      ])
      if_cmd.to_s
    end
  end
